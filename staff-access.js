...
(function(){
 ...
 // Low-cognitive-load employee management: self-contained cards, two primary actions,
 // a keyboard-accessible More menu for destructive/less frequent actions, and optional bulk actions.
...
@media(max-width:900px){.employee-access-table tr{grid-template-columns:1fr 1fr;grid-template-areas:"employee employee" "position status" "access access" "actions actions";gap:14px 18px}.employee-access-table td:nth-child(4){padding-top:0}.employee-access-table td:nth-child(5){margin-top:0}.employee-row-actions .employee-more-menu{right:auto;left:0;bottom:auto;top:calc(100% + 7px)}}
...
`;
   ...
   rows.forEach((row,index)=>{
     ...
     row.cells[1].dataset.label='Position';
     row.cells[2].dataset.label='Status';
     row.cells[3].dataset.label='Last access';
   ...
   selectAll.addEventListener('change',()=>{
     rows.filter(row=>!row.hidden).forEach(row=>{const checkbox=row.querySelector('.employee-select');if(checkbox)checkbox.checked=selectAll.checked;});
     updateSelection();
   });
   ...
   bulkReset.addEventListener('click',()=>{
     const selected=selectedRows();
     selected.forEach(row=>row.querySelector('[data-reset-password]')?.click());
     showToast(selected.length+' password reset request'+(selected.length===1?'':'s')+' recorded.');
     updateSelection();
   });
   bulkSuspend.addEventListener('click',()=>{
     const selected=selectedRows();
     selected.forEach(row=>{
       const suspend=row.querySelector('[data-suspend-access]');
       if(suspend&&!suspend.disabled&&suspend.textContent==='Suspend')suspend.click();
     });
     showToast(selected.length+' staff access action'+(selected.length===1?'':'s')+' processed.');
     updateSelection();
   });
   document.addEventListener('click',event=>{
     document.querySelectorAll('.employee-more[open]').forEach(menu=>{if(!menu.contains(event.target))menu.removeAttribute('open');});
   });
   updateSelection();
 }
