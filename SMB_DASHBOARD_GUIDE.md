...
## Consistent Settings sample data — September 8, 2026

All five existing Settings sections now use a coherent review dataset: Business Profile, Employees & Staff, Account/Roles/Approvals, Staff Roles/Access/Activity, and Notifications/Audit. settings-sample-data.js is the shared fixture source; HTML summary/table rows are materialized from it in this change. Employee search and staff directory consume its people array. Future fixture edits must regenerate the HTML snapshots and reconcile counts.

Totals: 12 staff records (11 employees plus 1 independent contractor), 9 active, 1 invited, 2 pending privileged approval; 5 distinct active roles; 3 pending owner requests; 12 sample audit events (9 role assignments plus 3 linked pending requests). Luis retains Employee / Staff permissions while his Supervisor promotion is pending. Mateo and Sofía have no permissions or sessions pending manual owner review. Example job scopes use least privilege for orders, inventory, logistics, accounting, catalog and business oversight. All access remains illustrative, not server authorization.
